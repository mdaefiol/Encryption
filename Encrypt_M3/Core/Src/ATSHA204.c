...
void WriteConfigZone(void){
	...
	uint8_t config_2[] = {0xC8, 0x00, 0x55, 0x00, 0xC1, 0x40, 0xc0, 0x50, 0xc0, 0x40, 0x80, 0x80, 0xd2, 0x55, 0x80, 0x40, 0xc5, 0x43,
						  0x85, 0x40, 0xc5, 0x43, 0xc5, 0x43, 0xc5, 0x43, 0x85, 0x83, 0xc5, 0x43, 0x85, 0x43, 0x05, 0x03, 0x85, 0x43};

	for (uint8_t i = 0; i < sizeof(config_2); i += 4){

		uint8_t configSlot[12] = {COMMAND, ((sizeof(configSlot)-1) & 0xFF), COMMAND_WRITE, 0x00, slot_addr + (i/4), 0x00};
		...
		configSlot[sizeof(configSlot) - 2] = CRC_receiv[0];
		configSlot[sizeof(configSlot) - 1] = CRC_receiv[1];
...
void WriteDataZone(uint8_t *slot_LSB, uint8_t *slot_data){
	...
	/*
	uint8_t writeData1[] = {COMMAND, SIZE_WRITE_DATA, COMMAND_WRITE, 0x82, 0x08, 0x00 , 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16,
							0x17, 0x18, 0x19, 0x1A, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x1A, 0x10, 0x11,
							0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, 0x33, 0x59};
...
void WriteOTPZone(void){
	...
	uint8_t CRC_receiv[2];
	uint8_t adress = 0x00;
	uint8_t OTP_Zone[4] = {0x00};
	// Write OTP command: {COMMAND, COUNT, OPCODE, ZONE, ADRESS, DADOS, CRC1, CRC2}

	for (uint8_t i = 0; i < 0x10; i++){

		uint8_t writeOTP[11] = {COMMAND, ((sizeof(writeOTP)-1) & 0xFF), COMMAND_WRITE, ZONE_OTP, (adress + i)};
...
void BlockDataZone(void){
	...
	uint8_t blockConfig[8] = {COMMAND, ((sizeof(blockConfig)-1) & 0xFF), COMMAND_LOCK, ZONE_DATA_LOCK, 0x04, 0x58};

	atCRC(blockConfig,sizeof(blockConfig), CRC_receiv);
	blockConfig[sizeof(blockConfig) - 2] = CRC_receiv[0] ;
	blockConfig[sizeof(blockConfig) - 1] = CRC_receiv[1] ;
...
void WriteEncript(void){
	...
	// Write command: {COMMAND, COUNT, OPCODE, Param1, Param2_LSB, Param2_MSB, DADOS, CRC_LSB, CRC_MSB}
	uint8_t writeEncript[] = {COMMAND, 0x55, COMMAND_WRITE, 0x82, 0x00, 0x00 , 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8,
							  0xA9, 0xAA, 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9, 0xAA, 0xA0, 0xA1,  0xA2, 0xA3,
							  0xA4, 0xA5, 0xA6, 0xA7, 0xA8, 0xA9,  /*0x15, 0xA9*/ 0x00, 0x00};
